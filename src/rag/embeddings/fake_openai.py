...
class FakeEmbeddingResponse:
    ...
    def __init__(self, embeddings: List[List[float]]):
        ...
        self.usage = {"prompt_tokens": len(embeddings) * 10, "total_tokens": len(embeddings) * 10}
    
    def model_dump(self) -> Dict[str, Any]:
        """Return dictionary representation of the response."""
        return {
            "data": [{"embedding": emb.embedding, "index": emb.index} for emb in self.data],
            "model": self.model,
            "usage": self.usage
        }
...
class FakeChatResponse:
    ...
    def model_dump(self) -> Dict[str, Any]:
        """Return dictionary representation of the response."""
        return {
            "choices": [{"message": {"content": choice.message.content, "role": choice.message.role}, 
                        "finish_reason": choice.finish_reason, "index": choice.index} for choice in self.choices],
            "model": self.model,
            "usage": self.usage
        }
...
class FakeEmbeddings:
    ...
    def create(self, input: List[str], model: str = "text-embedding-3-small") -> FakeEmbeddingResponse:
...
class FakeChatCompletions:
    ...
    def create(self, messages: List[Dict[str, Any]], model: str = "gpt-4", **kwargs) -> FakeChatResponse:
        """Create fake chat completion."""
        # Simple rule-based responses for testing
        last_message = messages[-1]["content"].lower()
        
        if "python" in last_message and ("who" in last_message or "created" in last_message):
            ...
            return FakeChatResponse("JavaScript is a programming language created by Brendan Eich.")
        else:
            return FakeChatResponse("I couldn't find any relevant information in the indexed documents.")
...
class FakeOpenAI:
    ...
    def __init__(self, api_key: Optional[str] = None, embedding_dimension: int = 1536):
