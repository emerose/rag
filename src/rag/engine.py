...
class RAGEngine:
    ...
    def __init__(self, 
        ...
        if self.runtime.progress_callback and not callable(self.runtime.progress_callback):
    ...
    @classmethod
    def create(cls, config: RAGConfig, runtime_options: Optional[RuntimeOptions] = None) -> 'RAGEngine':
    ...
    @classmethod
    def create_with_defaults(cls, documents_dir: str, cache_dir: str = '.cache') -> 'RAGEngine':
    ...
    def _initialize_embeddings(self) -> None:
        ...
        self._log("DEBUG", f"Using embedding model: {self.config.embedding_model} (version: {self.embedding_model_version})")
    ...
    def index_file(self, file_path: Union[Path, str]) -> bool:
        """Index a single file.
        ...
        try:
            ...
            if not existing_vectorstore:
                existing_vectorstore = self.vectorstore_manager.load_vectorstore(str_file_path)
            ...
            self._log("INFO", f"Successfully indexed {file_path} with {len(documents)} chunks")
            return True
    ...
    def index_directory(self, directory: Optional[Union[Path, str]] = None) -> Dict[str, bool]:
    ...
    def answer(self, question: str, k: int = 4) -> Dict[str, Any]:
        ...
        if not self.vectorstores:
            self._log("ERROR", "No indexed documents found. Please index documents first.")
            return {
                "question": question,
    ...
    def invalidate_cache(self, file_path: Union[Path, str]) -> None:
