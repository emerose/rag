...
from rag.data.metadata_extractor import (
...
def test_markdown_metadata_extraction(fixtures_dir: Path) -> None:
    ...
    assert metadata["title"] == "Sample Document with Headings", (
    ...
    for chapter in chapter_names:
        assert any(chapter in c for c in detected_chapters), (
            f"Chapter {chapter} not detected"
        )
