...
def test_document_processor_init() -> None:
    """Test initializing the DocumentProcessor.

    Verifies that DocumentProcessor properly initializes with its dependencies.
    """
    # Create mocks
...
def test_process_file() -> None:
    ...
    Verifies that a single file is properly processed through the entire pipeline:
    1. File validation
    2. Document loading
    3. Text splitting
...
def test_process_directory() -> None:
    ...
    Verifies that a directory of files is properly processed:
...
def test_enhance_documents() -> None:
    ...
    with (
        patch.object(
            processor.text_splitter_factory, "get_token_length", return_value=10
        ),
