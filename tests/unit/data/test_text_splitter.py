...
class TestSemanticRecursiveCharacterTextSplitter:
    ...
    def test_split_text(self) -> None:
        ...
        splitter = SemanticRecursiveCharacterTextSplitter(chunk_size=100, chunk_overlap=0)
        ...
        assert chunks[1] == "This is another paragraph with more text to ensure we hit the chunk boundary."
    ...
    def test_split_documents(self) -> None:
        ...
        assert split_docs[1].page_content == "This is another paragraph with more text to ensure we hit the chunk boundary."
...
class TestTextSplitterFactory:
    ...
    def test_token_based_chunking(self) -> None:
        ...
        long_text = "This is a test document with enough tokens to split into multiple chunks."
        ...
        for doc in split_docs:
            ...
            assert token_count <= 10, f"Chunk has {token_count} tokens, which exceeds the limit of 10"
        ...
        assert len(reconstructed) >= len(long_text) * 0.9, "Content was lost during chunking"
...
"""
        ...
        section_chunks = [doc for doc in split_docs if "## Section 1" in doc.page_content]
        if section_chunks:
        ...
        subsection_chunks = [doc for doc in split_docs if "### Subsection 1.1" in doc.page_content]
        ...
        content_chunks = [
            ...
            if "Content for section" in doc.page_content and "##" not in doc.page_content
        ]
        
        # Ensure all content chunks have heading metadata
        for chunk in content_chunks:
            assert "closest_heading" in chunk.metadata, "Content chunk missing closest_heading metadata"
            
        # Verify section 1 content has correct heading metadata
        section1_content = [
            doc for doc in content_chunks 
            if "Content for section 1" in doc.page_content
        ]
        if section1_content:
    ...
    def test_semantic_chunking_disabled(self) -> None:
        ...
        text = "First paragraph with some semantic content.\n\n" + \
        ...
        if token_count > factory.chunk_size:
            ...
            assert len(split_docs) > 1, "Document was not split despite token count exceeding chunk size"
...
"""
    ...
    def test_pdf_heading_extractor(self) -> None:
        ...
        with patch('os.path.isfile', return_value=True):
            ...
            with patch('rag.data.metadata_extractor.extract_pages') as mock_extract_pages:
                ...
                mock_page.__iter__.return_value = [text_element1, text_element2, text_element3]
        
                # Return the mocked page when extract_pages is called
                mock_extract_pages.return_value = [mock_page]
        
                # Also mock the LTTextContainer and LTChar classes
                with patch('rag.data.metadata_extractor.LTTextContainer', MagicMock):
                    with patch('rag.data.metadata_extractor.LTChar', MagicMock):
                        # Create a test Document
                        from langchain_core.documents import Document
                        test_doc = Document(page_content="Test content", metadata={"source": "mock_pdf.pdf"})
